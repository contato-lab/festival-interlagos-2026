...
def aggregate(movements):
    """Agrega movimentos por data da venda original, separado por Moto e Auto.

    Cancelamentos e estornos sao atribuidos a data da ISSUANCE original
    (via purchase.id), nao a data em que o cancelamento ocorreu. Isso garante
    que o numero de ingressos diarios "liquidos" bata com o dashboard oficial
    do Ticketmaster (que tambem aplica essa logica).
    """
    # Indexa todas as ISSUANCES por purchase.id -> date_str da venda original
    issuance_date_by_purchase = {}
    for mv in movements:
        if mv.get("operation") == "ISSUANCE":
            pid = (mv.get("purchase") or {}).get("id")
            if pid is not None and pid not in issuance_date_by_purchase:
                issuance_date_by_purchase[pid] = mv.get("date", "")[:10]
